...
async function _redeemProviderQuotaReset(card,button,status){
  ...
  if(requiresForce){
    const ok=await showConfirmDialog({
      ...
      message:t('provider_quota_reset_force_message'),
  ...
  card.innerHTML=`
    <div class="provider-quota-header">
      ...
      <div class="provider-quota-actions">
        ...
        ${bankedResetState.canRedeem?`<button class="provider-quota-refresh" type="button" data-provider-quota-reset>${esc(t('provider_quota_reset_action')+' ('+bankedResetState.count+')')}</button>`:''}
      </div>
    </div>
